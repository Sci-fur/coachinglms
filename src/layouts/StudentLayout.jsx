...
const sidebarItems = [
  ...
  { to: "/dashboard/add-course", icon: BookPlus, label: "Add Course" },
  { to: "/dashboard/courses", icon: BookOpen, label: "Course & Content" },
  { to: "/dashboard/master-class", icon: GraduationCap, label: "Master / Foundation Class" },
  { to: "/dashboard/past-classes", icon: History, label: "Live Classes" },
  { to: "/dashboard/exams", icon: FileCheck, label: "Past Exams / Practice" },
  { to: "/dashboard/solve-sheet", icon: FileSpreadsheet, label: "Solve Sheet" },
  { to: "/dashboard/performance", icon: BarChart3, label: "Performance" },
  { to: "/dashboard/qa", icon: HelpCircle, label: "Q&A Service" },
  { to: "/dashboard/payments", icon: CreditCard, label: "Due Payment" },
  { to: "/dashboard/community", icon: MessageCircle, label: "Discussion Group" },
];
      ...
      <header className="h-14 shrink-0 bg-white border-b border-[#E8ECF0] flex items-center justify-between px-4 z-30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-base hidden sm:inline text-slate-800">LMS</span>
          </div>
        </div>

        <div className="flex items-center gap-2 sm:gap-3">
          {user?.role === "admin" && (
            <Button variant="outline" size="sm" onClick={() => navigate("/admin")} className="text-xs h-8 border-[#E8ECF0] text-slate-600">
              Admin panel
            </Button>
          )}
          <div className="flex items-center gap-2 pl-2 border-l border-[#E8ECF0]">
